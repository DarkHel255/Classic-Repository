...
function getPrenomVisiteur($pdo, $idea)
{
	$req = "select prenom from visiteur where id = $idea"
	$res = $pdo->query($req);
	$ligne = $res->fetch();
	return $ligne;
}
...
function getNomVisiteur($pdo, $idea)
{
	$req = "select nom from visiteur where id = $idea"
	$res = $pdo->query($req);
	$ligne = $res->fetch();
	return $ligne;
}
